...
DATA = {'title': 'Midpoint Method',
 'label': 'SECOND-ORDER RUNGE-KUTTA METHOD',
 'solver': 'Midpoint_Solver',
 'quiz': 'Midpoint_Quiz',
 'hero': "Improve Euler's approximation by estimating the slope at the midpoint of every step.",
 'summary': [('METHOD TYPE', 'Explicit RK2 method'),
             ('SLOPE EVALUATIONS', 'Two per step'),
             ('GLOBAL ORDER', 'Second order, O(h²)'),
             ('MIDPOINT', 'Controls the update')],
 'overview': 'The explicit Midpoint Method is a second-order Runge-Kutta method for y′=f(x,y). It '
             'first predicts a midpoint state using the starting slope, then uses the slope at '
             'that midpoint to advance the full step.',
 'foundation': 'A midpoint slope represents the average behavior over a short interval more '
               'accurately than the starting slope alone. The resulting method matches the Taylor '
               'expansion through terms of order h².',
 'foundation_formula': 'k_1=f(x_n,y_n),\\qquad k_2=f\\!\\left(x_n+\\frac h2,y_n+\\frac '
                       'h2k_1\\right)',
 'conditions': ['A first-order initial-value problem must be specified.',
                'The function f should be continuous and sufficiently smooth over the step.',
                'A consistent step size and target direction must be chosen.',
                'The method is the explicit midpoint RK2 method, not the implicit midpoint '
                'method.'],
 'formula_title': 'Midpoint Update',
 'formula': 'y_{n+1}=y_n+h\\,k_2,\\quad k_2=f\\!\\left(x_n+\\frac h2,y_n+\\frac h2 '
            'f(x_n,y_n)\\right)',
 'formula_note': 'The full-step update uses the slope evaluated at the estimated midpoint.',
 'accuracy_title': 'Error Order',
 'accuracy_formula': '\\text{local truncation error}=O(h^3),\\qquad \\text{global error}=O(h^2)',
 'accuracy_text': 'Halving h usually reduces the dominant global error by about a factor of four.',
 'comparison_title': 'Compared with Euler',
 'comparison_text': 'Both methods are explicit, but midpoint uses one additional slope evaluation '
                    'and is usually much more accurate for the same step size.',
 'steps': ['Record the ODE, initial condition, and step size.',
           'Compute k₁=f(xₙ,yₙ).',
           'Estimate the midpoint xₙ+h/2 and yₙ+(h/2)k₁.',
           'Evaluate k₂ at the estimated midpoint.',
           'Update yₙ₊₁=yₙ+h k₂ and xₙ₊₁=xₙ+h.',
           'Repeat for all steps.'],
 'example_title': 'One Midpoint Step',
 'example_setup': "y'=x+y,\\quad y(0)=1,\\quad h=0.1",
 'example_lines': ['k_1=f(0,1)=1',
                   '(x_{\\mathrm{mid}},y_{\\mathrm{mid}})=(0.05,1.05)',
                   'k_2=f(0.05,1.05)=1.10',
                   'y_1=1+0.1(1.10)=1.11'],
 'question': 'For the same sufficiently small h, how does midpoint usually compare with Euler?',
 'options': ['Midpoint is generally more accurate.',
             'Euler is always more accurate.',
             'They always give identical values.'],
 'answer': 'Midpoint is generally more accurate.',
 'answer_explanation': 'Midpoint is second-order globally, while Euler is first-order.',
 'python': 'def midpoint_method(f, x0, y0, h, steps):\n'
           '    x = float(x0)\n'
           '    y = float(y0)\n'
           '    values = [(0, x, y)]\n'
           '\n'
           ...
           '\n'
           '    return values',
 'matlab': 'function values = MidpointMethod(f,x0,y0,h,steps)\n'
           'x = x0;\n'
           'y = y0;\n'
           ...
           'end\n'
           'end',
 'applications': [('Trajectory Approximation', 'Model smooth motion more accurately than Euler.'),
                  ('Population and Chemical Models',
                   'Advance nonlinear initial-value problems with moderate cost.'),
                  ('Embedded Simulations',
                   'Use a simple second-order method when RK4 is unnecessarily expensive.')],
 'advantages': ['Second-order accuracy with only two slope evaluations.',
                'Simple explicit update.',
                'Substantially more accurate than Euler for many smooth problems.',
                'Useful introduction to Runge-Kutta construction.'],
 'limitations': ['Still unsuitable for many stiff problems.',
                 'Requires twice as many function evaluations as Euler.',
                 'Accuracy depends strongly on step size and smoothness.',
                 'The explicit and implicit midpoint methods must not be confused.'],
 'footer': 'Midpoint Method • Ordinary Differential Equations'}
