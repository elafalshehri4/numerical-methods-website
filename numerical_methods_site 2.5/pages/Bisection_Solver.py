...
def analyze_interval_domain(
    ...
    """Detect obvious domain/discontinuity problems by numerical sampling.

    Sampling cannot prove continuity mathematically, but it catches common invalid
    brackets such as log(x) across x <= 0 and 1/x across x = 0.
    """
    ...
    if np.count_nonzero(np.isfinite(sample_y)) != sample_y.size:
        raise ValueError(
            ...
            "sampled points inside [a, b]. Choose an interval where the function "
    ...
    if finite_differences.size:
        ...
        if maximum_difference > max(1.0e6 * max(median_difference, 1.0e-15), 1.0e6 * scale):
            warnings.append(
...
def solve_by_bisection(
    ...
    try:
        ...
        warnings = analyze_interval_domain(function, a0, b0)
        ...
        for iteration in range(1, imax + 1):
            ...
            residual_satisfied = residual <= FUNCTION_ZERO_TOLERANCE
            ...
            if error_satisfied:
                ...
                stopping_reason = (
                    ...
                    "satisfied the requested stopping criterion εs."
                )
                break
        ...
        message = (
            ...
            if converged
            else "Maximum iterations reached. The final approximation is shown."
        )
