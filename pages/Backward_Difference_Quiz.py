...
DATA = {'title': 'Backward Difference Quiz',
 'label': 'BACKWARD DIFFERENCE QUIZ',
 'lesson': 'Backward_Difference',
 'solver': 'Backward_Difference_Solver',
 'category': 'Numerical Differentiation',
 'key': 'backward_difference',
 'description': 'Test the backward first-derivative formula, one-sided data use, error order, boundaries, and '
                'step-size effects.',
 'questions': [('The backward first-difference formula approximates:',
                ['A) f′(x)', 'B) ∫f(x)dx', 'C) A root of f', 'D) An eigenvalue'],
                'A'),
               ('The first-order backward formula is:',
                ['A) [f(x+h)-f(x)]/h', 'B) [f(x)-f(x-h)]/h', 'C) [f(x+h)-f(x-h)]/(2h)', 'D) [f(x)+f(x-h)]/h'],
                'B'),
               ('Backward Difference uses values at:',
                ['A) x and x+h', 'B) x-h and x', 'C) x-h and x+h', 'D) x only'],
                'B'),
               ('The standard backward formula is:',
                ['A) One-sided', 'B) Two-sided', 'C) Implicit', 'D) An integration rule'],
                'A'),
               ('For a sufficiently smooth function, its leading truncation error is:',
                ['A) O(h)', 'B) O(h²)', 'C) O(h³)', 'D) O(1/h)'],
                'A'),
               ('Backward Difference is especially useful near:',
                ['A) A left boundary where x-h is unavailable',
                 'B) A right boundary where x+h is unavailable',
                 'C) Only the center of a grid',
                 'D) A matrix pivot'],
                'B'),
               ('The symbol h represents the:',
                ['A) Step size', 'B) Exact derivative', 'C) Number of variables', 'D) Residual'],
                'A'),
               ('Compared with the centered first difference at the same h, the basic backward difference is '
                'generally:',
                ['A) Less accurate', 'B) More accurate', 'C) The same order', 'D) Exact for every function'],
                'A'),
               ('If f(x)=x², the backward approximation at x=2 with h=0.1 is:',
                ['A) 3.9', 'B) 4.0', 'C) 4.1', 'D) 4.2'],
                'A'),
               ('For f(x)=x², the exact derivative at x=2 is:', ['A) 2', 'B) 4', 'C) 6', 'D) 8'], 'B'),
               ('Reducing h usually reduces truncation error until:',
                ['A) Round-off and cancellation become significant',
                 'B) The function becomes linear',
                 'C) The derivative becomes an integral',
                 'D) The interval doubles'],
                'A'),
               ('Which Taylor expansion produces the backward formula?',
                ['A) Expansion of f(x-h) about x',
                 'B) Expansion of f(x+h) only',
                 'C) Expansion of an antiderivative',
                 'D) Expansion of a matrix inverse'],
                'A'),
               ('The basic backward difference is exact for:',
                ['A) Linear functions', 'B) All quadratics', 'C) All cubics', 'D) Every smooth function'],
                'A'),
               ('Which data requirement is essential?',
                ['A) A function value at x-h',
                 'B) A value at x+h only',
                 'C) A Jacobian matrix',
                 'D) An antiderivative'],
                'A'),
               ('The main trade-off in choosing h is between:',
                ['A) Truncation error and round-off error',
                 'B) Matrix size and determinant',
                 'C) Roots and eigenvalues',
                 'D) Integration and interpolation'],
                'A')]}
