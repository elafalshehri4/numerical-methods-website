...
def run_euler_grid(
...
) -> tuple[float, tuple[EulerIteration, ...]]:
    """Run the standard explicit Euler method on a uniform grid."""
    ...
    for iteration in range(1, steps + 1):
        derivative = evaluate_ode(ode_function, current_x, current_y)
        increment = step_size * derivative
        next_x = x0 + iteration * step_size
        next_y = current_y + increment

        if not math.isfinite(next_x) or not math.isfinite(next_y):
            raise ValueError(
                f"Euler's method produced a non-finite approximation at iteration {iteration}."
            )
        ...
        if store_history:
            history.append(
                EulerIteration(
                    ...
                    euler_increment=float(increment),
                    x_next=float(next_x),
                    y_next=float(next_y),
                    ...
                    operation=(
                        ...
                        f"({step_size:.12g})({derivative:.12g}) = {next_y:.12g}"
                    ),
                    status="Completed",
                )
            )
...
def build_convergence_analysis(
    ...
    """Refine the Euler grid and estimate first-order convergence."""

    raw_records: list[dict[str, float | int | None]] = []

    for level in range(1, levels + 1):
        steps = base_steps * (2 ** (level - 1))
        final_y, _ = run_euler_grid(
...
def error_result(message: str, input_signature: str = "") -> EulerResult:
    ...
    try:
        ...
        if exact_function is not None:
            ...
            residual_expression = sp.simplify(
                exact_derivative
                - ode_expression.subs(Y_SYMBOL, exact_expression)
            )
            if residual_expression != 0:
                try:
                    ...
                    residual_values = [
                        abs(validate_finite_real(residual_function(point), "Exact solution residual"))
        ...
        final_y, iterations = run_euler_grid(
            ode_function=ode_function,
            x0=x0,
            y0=y0,
            x_end=x_end,
        ...
        if any(abs(record.y_next) >= VALUE_MAGNITUDE_WARNING for record in iterations):
            warnings.append(
                "The numerical solution reached a very large magnitude. Euler's method "
                "may be unstable for this step size or differential equation."
            )

        if latest_observed_order is not None and latest_observed_order < 0.5:
            warnings.append(
                "The observed refinement behavior is weaker than the expected first-order "
                "convergence. Reduce the step size and inspect possible instability."
            )
        ...
        return EulerResult(
            ...
            stopping_reason=(
                "The requested number of uniform Euler steps was completed and the "
                "final grid point was reached."
            ),
        ...
        rows.append(
            {
                ...
                "x_n": record.x_n,
                "y_n": record.y_n,
                "f(x_n, y_n)": record.derivative,
                "h": record.step_size,
                "Euler Increment": record.euler_increment,
    ...
    rows = [
        {
            "Point": 0,
            "x": result.x0,
            "Euler Approximation": result.y0,
            "Exact Solution": (
                evaluate_exact_solution(
                    sp.lambdify(X_SYMBOL, result.exact_solution_expression, modules=["numpy"]),
                    float(result.x0),
                )
        ...
        rows.append(
            {
                ...
                "x": record.x_next,
                "Euler Approximation": record.y_next,
...
def create_solution_figure(result: EulerResult) -> Figure:
    """Plot the Euler solution and optional exact solution."""
    ...
    axis.plot(
        solution_df["x"],
        solution_df["Euler Approximation"],
        marker="o",
        linewidth=2,
    ...
    axis.scatter(
        ...
        s=110,
        marker="*",
        label=f"Final Euler Value = {result.final_y:.6g}",
        zorder=6,
    )
    axis.axhline(0.0, linewidth=1)
    axis.axvline(0.0, linewidth=1)
    axis.set_title("Euler Method Numerical Solution")
    axis.set_xlabel("x")
    axis.set_ylabel("y")
    axis.grid(True)
    axis.legend()
...
def create_error_figure(result: EulerResult) -> Figure | None:
    ...
    axis.set_title("Euler Method Global Error")
    axis.set_xlabel("x")
...
def create_convergence_figure(result: EulerResult) -> Figure:
    ...
    valid = np.isfinite(metric) & (metric > 0.0) & np.isfinite(h_values) & (h_values > 0.0)
    ...
    if np.any(valid):
        ...
        reference_line = reference_metric[-1] * (reference_h / reference_h[-1])
        axis.loglog(
            reference_h,
            reference_line,
            linestyle="--",
    ...
    axis.set_title("Euler Method Grid-Refinement Convergence")
    axis.set_xlabel("|h|")
...
def generate_excel_report(result: EulerResult) -> bytes:
    ...
    summary_rows = [
        ...
        ("Final Euler Approximation", result.final_y),
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if len(result.iterations) > 0:
            chart = LineChart()
            chart.title = "Euler Numerical Solution"
            ...
            chart.height = 8
            chart.width = 15
            iteration_sheet.add_chart(chart, "Q2")
...
def render_final_result(result: EulerResult) -> None:
    ...
    metric_columns[1].metric("Euler Approximation", format_number(result.final_y))
...
def render_method_summary(result: EulerResult) -> None:
    ...
    with column_1:
        ...
        st.latex(r"h=\frac{x_{end}-x_0}{N}")
    with column_2:
        st.latex(r"y_{n+1}=y_n+h f(x_n,y_n)")
...
def render_iteration_table(result: EulerResult) -> None:
    """Render the full Euler step table and detailed operations."""
    ...
    with st.expander("Detailed Euler Operations"):
        for record in result.iterations:
            ...
            st.caption(
                f"Slope f(x_n, y_n) = {record.derivative:.12g}; "
                f"increment h·f = {record.euler_increment:.12g}."
            )


def render_solution_graph(result: EulerResult) -> None:
...
def render_convergence_analysis(result: EulerResult) -> None:
    ...
    st.markdown(
        "Euler's method has **first-order global convergence** under the usual "
        "smoothness and stability conditions. Halving the step size should "
        "approximately halve the global error once the asymptotic range is reached."
    )
...
def render_excel_download(result: EulerResult) -> None:
    ...
    filename = f"euler_method_report_{timestamp}.xlsx"
    st.download_button(
    ...
    st.html(
        ...
        <section class="solver-hero">
            <div>
                ...
                <p>
                    Solve a first-order initial-value problem using the standard
                    explicit Euler Method. Review every slope evaluation, update,
                    numerical solution point, optional exact-solution error,
    ...
    with main_area:
        ...
        with conditions_column:
            with st.container(border=True):
                ...
                - Euler’s Method is first-order accurate, so a smaller step size is often needed for good accuracy.
                - The optional exact solution must satisfy the supplied initial condition.
                - Discontinuities, singularities, or unstable step sizes can cause the numerical solution to fail.
                    """
                )
        ...
        with input_column:
            with st.container(border=True):
                ...
                with first_row[1]:
                    st.markdown(
                        '<div class="input-label-ui">Initial y₀</div>',
                        unsafe_allow_html=True,
                    )
                    y0 = st.number_input(
                        "Initial y0",
        ...
        with result_column:
            with st.container(border=True):
                ...
                if saved_result is None:
                    ...
                    st.warning(
                        "The inputs have changed. Select Solve with Euler Method to update the result."
                    )
                else:
