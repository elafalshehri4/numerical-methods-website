    ...
    with main_area:
        ...
        with guide_column:
            with st.container(border=True):
                ...
                st.markdown(
                    """
                Enter the known data as paired numerical values **(xᵢ, yᵢ)**.

                - Each row represents one interpolation node.
                - All values must be finite and every **x** value must be distinct.
                - Use at least two points.
                - Enter the target **x** value at which the polynomial will be evaluated.
                - The optional exact function is used only to compare the interpolation result with a reference.
                    """
                )
        ...
        with conditions_column:
            with st.container(border=True):
                ...
                st.markdown(
                    """
                - **n** distinct nodes determine one polynomial of degree at most **n − 1**.
                - Duplicate **x** values make the interpolation system singular.
                - Closely clustered or widely separated nodes may produce a poorly conditioned system.
                - Extrapolation outside the node range can have much larger error than interpolation.
                    """
                )
