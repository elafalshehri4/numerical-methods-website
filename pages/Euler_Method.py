...
DATA = {'title': 'Euler Method',
 'label': 'ORDINARY DIFFERENTIAL EQUATION METHOD',
 'solver': 'Euler_Solver',
 'quiz': 'Euler_Quiz',
 'hero': 'Approximate a first-order initial-value problem one step at a time by following the '
         'slope at the current point.',
 'summary': [('METHOD TYPE', 'Explicit one-step ODE method'),
             ('SLOPE EVALUATIONS', 'One per step'),
             ('GLOBAL ORDER', 'First order, O(h)'),
             ('INITIAL VALUE', 'Required')],
 'overview': "Euler's Method approximates the solution of an initial-value problem y′ = f(x,y), "
             'y(x₀)=y₀. Starting from a known point, it follows the tangent line over a short step '
             'h to estimate the next value.',
 'foundation': 'A first-order Taylor expansion gives y(x+h)=y(x)+h y′(x)+O(h²). Replacing y′ by '
               "f(x,y) and discarding the higher-order terms produces Euler's update.",
 'foundation_formula': 'y(x+h)=y(x)+h\\,f(x,y)+O(h^2)',
 'conditions': ['A first-order ODE y′ = f(x,y) and an initial value y(x₀)=y₀ must be given.',
                'The function f should be continuous near the solution; local Lipschitz continuity '
                'in y supports uniqueness and stable step-by-step propagation.',
                'The step size must move in the direction of the target point and should be small '
                'enough for the required accuracy.',
                'The method is explicit, so no nonlinear equation is solved at each step.'],
 'formula_title': 'Euler Update',
 'formula': 'y_{n+1}=y_n+h\\,f(x_n,y_n),\\qquad x_{n+1}=x_n+h',
 'formula_note': 'The current slope is assumed to remain approximately constant over one step.',
 'accuracy_title': 'Error Order',
 'accuracy_formula': '\\text{local truncation error}=O(h^2),\\qquad \\text{global error}=O(h)',
 'accuracy_text': 'Halving h usually reduces the accumulated global error by about a factor of '
                  'two, provided round-off and instability are not dominant.',
 'comparison_title': 'Interpretation',
 'comparison_text': 'Euler is the simplest Runge-Kutta method. It is useful for learning and rough '
                    'estimates, but midpoint, Heun, and RK4 are usually more accurate for the same '
                    'h.',
 'steps': ['Write y′=f(x,y) and record (x₀,y₀).',
           'Choose a step size h and determine the number of steps.',
           'Evaluate the current slope f(xₙ,yₙ).',
           'Compute yₙ₊₁=yₙ+h f(xₙ,yₙ).',
           'Update xₙ₊₁=xₙ+h.',
           'Repeat until the target point is reached.'],
 'example_title': 'One Euler Step',
 'example_setup': "y'=x+y,\\quad y(0)=1,\\quad h=0.1",
 'example_lines': ['f(0,1)=1',
                   'y_1=1+0.1(1)=1.1',
                   'x_1=0.1',
                   '\\text{Exact }y(0.1)=2e^{0.1}-1.1\\approx1.110342'],
 'question': "What usually happens to Euler's global error when the step size is halved?",
 'options': ['It is roughly halved.', 'It is roughly quartered.', 'It always becomes zero.'],
 'answer': 'It is roughly halved.',
 'answer_explanation': 'Euler is first-order globally, so the dominant global error is '
                       'proportional to h.',
 'python': 'def euler_method(f, x0, y0, h, steps):\n'
           '    if steps < 0:\n'
           ...
           '\n'
           '    return values',
 'matlab': 'function values = EulerMethod(f,x0,y0,h,steps)\n'
           'x = x0;\n'
           'y = y0;\n'
           ...
           'end\n'
           'end',
 'applications': [('Population Models', 'Quickly approximate simple growth or decay models.'),
                  ('Motion Models',
                   'Advance position or velocity equations from known initial data.'),
                  ('Teaching and Prototyping',
                   'Illustrate how derivative information generates a numerical trajectory.')],
 'advantages': ['Very simple to understand and implement.',
                'Requires only one function evaluation per step.',
                'Provides a baseline for comparing higher-order ODE methods.',
                'Works with nonlinear right-hand sides without symbolic integration.'],
 'limitations': ['Only first-order accurate globally.',
                 'Can require very small steps for acceptable accuracy.',
                 'May be unstable for stiff differential equations.',
                 'Error accumulates over many steps.'],
 'footer': 'Euler Method • Ordinary Differential Equations'}
