...
DATA = {'title': 'Trapezoidal Rule',
 'label': 'NUMERICAL INTEGRATION METHOD',
 'solver': 'Trapezoidal_Solver',
 'quiz': 'Trapezoidal_Quiz',
 'hero': 'Approximate a definite integral by replacing the curve with one or more straight-sided '
         'trapezoids.',
 'summary': [('METHOD TYPE', 'Closed Newton-Cotes rule'),
             ('GRID', 'Equally spaced points'),
             ('GLOBAL ORDER', 'Second order, O(h²)'),
             ('EXACTNESS', 'Polynomials through degree 1')],
 'overview': 'The Trapezoidal Rule approximates the area under a curve by connecting neighboring '
             'function values with straight lines. The composite rule adds the areas of multiple '
             'trapezoids across the interval.',
 'foundation': 'On each subinterval, the function is replaced by its linear interpolant. '
               'Integrating that line gives the trapezoid area. Summing all subinterval '
               'contributions produces the composite formula.',
 'foundation_formula': '\\int_{x_i}^{x_{i+1}} f(x)\\,dx\\approx\\frac '
                       'h2\\left[f(x_i)+f(x_{i+1})\\right]',
 'conditions': ['The integration limits a and b must be specified.',
                'For the composite formula, the interval is divided into n positive equal '
                'subintervals.',
                'The function should be finite and sufficiently smooth on the interval.',
                'Tabulated data may be used when the x-values are equally spaced.'],
 'formula_title': 'Composite Trapezoidal Rule',
 'formula': 'T_n=\\frac h2\\left[f(x_0)+2\\sum_{i=1}^{n-1}f(x_i)+f(x_n)\\right],\\qquad '
            'h=\\frac{b-a}{n}',
 'formula_note': 'Interior values are counted twice because each belongs to two adjacent '
                 'trapezoids.',
 'accuracy_title': 'Error Order',
 'accuracy_formula': "E_T=-\\frac{(b-a)}{12}h^2 f''(\\xi)\\quad\\text{for some }\\xi\\in(a,b)",
 'accuracy_text': 'For sufficiently smooth f, the composite error is O(h²). Halving h usually '
                  'reduces the dominant error by about four.',
 'comparison_title': 'When It Is Strong',
 'comparison_text': 'The rule is simple and works directly with tabulated data. For smooth curved '
                    "functions, Simpson's rule is often more accurate at comparable grid sizes.",
 'steps': ['Define f(x) and the interval [a,b].',
           'Choose a positive number n of equal subintervals.',
           'Compute h=(b-a)/n and the nodes xᵢ=a+ih.',
           'Evaluate f at all n+1 nodes.',
           'Apply endpoint weights 1 and interior weights 2.',
           'Multiply the weighted sum by h/2.'],
 'example_title': 'Composite Trapezoidal Example',
 'example_setup': '\\int_0^2 x^2\\,dx,\\qquad n=4,\\qquad h=0.5',
 'example_lines': ['x_i=0,0.5,1,1.5,2',
                   'f(x_i)=0,0.25,1,2.25,4',
                   'T_4=\\frac{0.5}{2}[0+2(0.25+1+2.25)+4]=2.75',
                   '\\text{Exact value}=\\frac83\\approx2.666667'],
 'question': 'Why are interior function values multiplied by 2?',
 'options': ['Each interior node belongs to two neighboring trapezoids.',
             'The method is fourth-order.',
             'Endpoints are not evaluated.'],
 'answer': 'Each interior node belongs to two neighboring trapezoids.',
 'answer_explanation': 'When the individual trapezoid areas are added, every interior endpoint '
                       'appears twice.',
 'python': 'import numpy as np\n'
           ...
           '        + 0.5 * y[-1]\n'
           '    )',
 'matlab': 'function I = CompositeTrapezoidal(f,a,b,n)\n'
           'if n <= 0 || floor(n) ~= n\n'
           ...
           'end',
 'applications': [('Measured Data', 'Integrate equally spaced experimental observations.'),
                  ('Engineering Areas and Work',
                   'Estimate accumulated quantities from sampled curves.'),
                  ('Baseline Integration',
                   'Provide a simple result for comparison with higher-order rules.')],
 'advantages': ['Simple and computationally inexpensive.',
                'Works with tabulated data.',
                'Composite refinement is straightforward.',
                'Exact for constant and linear functions.'],
 'limitations': ['Only second-order accurate for smooth functions.',
                 'Curvature creates systematic error on coarse grids.',
                 'Equal spacing is required by the standard composite formula.',
                 'Discontinuities or singularities require special treatment.'],
 'footer': 'Trapezoidal Rule • Numerical Integration'}
