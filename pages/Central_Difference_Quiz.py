...
DATA = {'title': 'Central Difference Quiz',
 'label': 'CENTRAL DIFFERENCE QUIZ',
 'lesson': 'Central_Difference',
 'solver': 'Central_Difference_Solver',
 'category': 'Numerical Differentiation',
 'key': 'central_difference',
 'description': 'Test the centered first-derivative formula, second-order accuracy, interior-point requirements, and '
                'step-size effects.',
 'questions': [('The centered first-difference formula approximates:',
                ['A) f′(x)', 'B) ∫f(x)dx', 'C) A root of f', 'D) An eigenvalue'],
                'A'),
               ('The centered first-difference formula is:',
                ['A) [f(x+h)-f(x)]/h', 'B) [f(x)-f(x-h)]/h', 'C) [f(x+h)-f(x-h)]/(2h)', 'D) [f(x+h)+f(x-h)]/(2h)'],
                'C'),
               ('Central Difference uses values at:',
                ['A) x and x+h', 'B) x-h and x', 'C) x-h and x+h', 'D) x only'],
                'C'),
               ('The standard central formula is:',
                ['A) One-sided', 'B) Two-sided', 'C) An integration rule', 'D) A root-finding method'],
                'B'),
               ('For a sufficiently smooth function, its leading truncation error is:',
                ['A) O(h)', 'B) O(h²)', 'C) O(h³)', 'D) O(1/h)'],
                'B'),
               ('Central Difference is normally best suited to:',
                ['A) Interior grid points',
                 'B) A left endpoint with no x-h value',
                 'C) A right endpoint with no x+h value',
                 'D) Singular matrices'],
                'A'),
               ('Why is the centered formula second-order?',
                ['A) The first-order Taylor error terms cancel',
                 'B) It uses no function values',
                 'C) It divides by h²',
                 'D) It is exact for every polynomial'],
                'A'),
               ('If f(x)=x³, the centered approximation at x=2 with h=0.1 is:',
                ['A) 11.99', 'B) 12.00', 'C) 12.01', 'D) 12.10'],
                'C'),
               ('For f(x)=x³, the exact derivative at x=2 is:', ['A) 4', 'B) 8', 'C) 12', 'D) 16'], 'C'),
               ('Halving h ideally reduces the dominant centered-difference truncation error by about:',
                ['A) 2', 'B) 4', 'C) 8', 'D) 16'],
                'B'),
               ('Reducing h too far can worsen the result because of:',
                ['A) Subtractive cancellation and round-off',
                 'B) Quadratic convergence',
                 'C) Pivoting',
                 'D) Interpolation exactness'],
                'A'),
               ('The centered formula requires:',
                ['A) Data on both sides of x',
                 'B) Only the value at x',
                 'C) An analytical antiderivative',
                 'D) A bracketing interval'],
                'A'),
               ('Compared with basic forward and backward differences at the same h, central difference is '
                'generally:',
                ['A) Higher order', 'B) Lower order', 'C) The same first order', 'D) Not a finite-difference method'],
                'A'),
               ('The centered first difference is exact for polynomials through degree:',
                ['A) 0', 'B) 1', 'C) 2', 'D) 3'],
                'C'),
               ('The main step-size trade-off is between:',
                ['A) Truncation error and round-off error',
                 'B) Matrix rank and determinant',
