...
FORMULA_OPTIONS = {
    "Two-point backward difference — first-order O(h)": "two_point",
    "Three-point backward difference — second-order O(h²)": "three_point",
}
...
# =============================================================================
# Numerical Backward Difference algorithm
# =============================================================================
...
def calculate_backward_difference(
    ...
    f_x_minus_h = evaluate_real_scalar(numeric_function, x_minus_h, "f(x₀ − h)")

    if formula_key == "two_point":
        numerator = f_x - f_x_minus_h
        denominator = step_size
        derivative = numerator / denominator
        x_minus_2h = None
        f_x_minus_2h = None
        substitution = (
            f"[{format_number(f_x, 10)} − {format_number(f_x_minus_h, 10)}] "
            f"/ {format_number(step_size, 10)}"
        )
    elif formula_key == "three_point":
        x_minus_2h = x_value - 2.0 * step_size
        if not math.isfinite(x_minus_2h):
        ...
        numerator = 3.0 * f_x - 4.0 * f_x_minus_h + f_x_minus_2h
        denominator = 2.0 * step_size
        derivative = numerator / denominator
        substitution = (
            f"[3({format_number(f_x, 10)}) − "
            f"4({format_number(f_x_minus_h, 10)}) + "
...
def solve_backward_difference(
    ...
    try:
        ...
        if abs(primary.f_x_minus_h - primary.f_x) <= ZERO_TOLERANCE:
            warnings.append(
...
def create_function_plot(result: BackwardDifferenceResult) -> plt.Figure:
    """Plot the function, sampled backward points, and approximate tangent line."""
    ...
    axis.scatter(
        ...
        [primary.f_x_minus_h],
        s=70,
        label="Backward point x₀ − h",
    )
    if primary.x_minus_2h is not None and primary.f_x_minus_2h is not None:
        axis.scatter(
            [primary.x_minus_2h],
            [primary.f_x_minus_2h],
            s=70,
            label="Backward point x₀ − 2h",
        )

    axis.set_title("Backward Difference — Function and Approximate Tangent")
    axis.set_xlabel("x")
    axis.set_ylabel("f(x)")
        ...
        <section class="solver-hero">
            <div>
                ...
                <p>
                    ...
                    the first derivative using standard backward finite-difference
    ...
    with main_area:
        ...
        with conditions_column:
            with st.container(border=True):
                ...
                - Backward differences are useful near a right boundary where values to the right are unavailable.
        ...
        with input_column:
            with st.container(border=True):
                ...
                st.caption(
                    ...
                    "be real and defined at all required backward points."
                )
