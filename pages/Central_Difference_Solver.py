...
# Constants
# =============================================================================
METHOD_NAME = "Central Difference Method"
DISPLAY_DECIMALS = 3
...
ACCURACY_OPTIONS = {
    "Standard centered Taylor formula — O(h²)": "standard",
    "Higher-accuracy centered Taylor formula — O(h⁴)": "high_accuracy",
}
...
def format_display_number(
    ...
    """Use fixed notation normally and ×10ⁿ for very small or large values."""
...
def format_number(
    ...
    """Format displayed numerical values consistently."""

    return format_display_number(value, decimals)


def scientific_number(value: float | int | None) -> str:
    """Format a value using three-decimal scientific notation."""
...
def round_numeric_dataframe(
    ...
    """Round numeric columns for display only; calculations keep full precision."""
...
# =============================================================================
# Textbook central-difference formulas derived from Taylor series
# =============================================================================
def get_formula_spec(
    derivative_order: int,
    accuracy_key: str,
) -> FormulaSpec:
    """Return centered finite-difference formulas from Chapra and Canale."""
...
def format_offset_expression(offset: int, variable: str = "x₀") -> str:
    """Return a readable centered-grid point such as x₀−2h or x₀+h."""

    if offset == 0:
        return variable
    sign = "+" if offset > 0 else "−"
    magnitude = abs(offset)
    factor = "" if magnitude == 1 else str(magnitude)
    return f"{variable}{sign}{factor}h"


def calculate_central_difference(
    numeric_function: Callable[[Any], Any],
    x_value: float,
    step_size: float,
    formula_spec: FormulaSpec,
) -> dict[str, Any]:
    """Calculate one centered finite-difference approximation manually."""
    ...
    for offset in formula_spec.offsets:
        ...
        if not math.isfinite(sample_point):
            raise ValueError("A required central sample point is not finite.")
        point_label = format_offset_expression(offset)
    ...
    if not all(
        math.isfinite(value)
        for value in (numerator, denominator, approximation)
    ):
...
def safe_relative_error(
    ...
    """Calculate dimensionless relative true error safely."""
...
def solve_central_difference(
    ...
    """Perform the complete validated Central Difference workflow."""
    ...
    try:
        ...
        for level in range(refinement_levels):
            ...
            calculation = calculate_central_difference(
                numeric_function,
                x_value,
                step_size,
                formula_spec,
        ...
        reliable_orders: list[float] = []
        for index, item in enumerate(evaluations):
            if index == 0 or item.observed_order is None:
                continue
            previous_error = evaluations[index - 1].absolute_error
            current_error = item.absolute_error
            if (
                ...
                and item.observed_order > 0.0
            ):
                reliable_orders.append(float(item.observed_order))

        latest_order = (
            float(np.median(reliable_orders))
            if reliable_orders
            else None
        )
...
def create_evaluation_dataframe(
    result: CentralDifferenceResult,
) -> pd.DataFrame:
    ...
    for item in result.evaluations:
        ...
        for offset, point, value in zip(
            ...
            point_label = format_offset_expression(offset, "x0")
            row[point_label] = point
            row[f"f({point_label})"] = value
        rows.append(row)
    ...
    for offset, coefficient, point, value in zip(
        ...
        rows.extend(
            [
                ...
                (format_offset_expression(offset), point),
                (f"f({format_offset_expression(offset)})", value),
            ]
        )
    rows.extend(
        [
...
def create_taylor_series_dataframe(
    ...
    for offset in result.formula_spec.offsets:
        rows.append(
            {
                "Offset k": offset,
                "Sample": f"f({format_offset_expression(offset, 'x0')})",
                ...
                "Taylor Expansion to Third Degree": (
                    f"f({format_offset_expression(offset, 'x0')}) = f(x0) + "
                    f"({offset}h)f'(x0) + ({offset}h)^2/2! f''(x0) + "
...
# Plots
# =============================================================================
def create_function_plot(result: CentralDifferenceResult) -> plt.Figure:
    """Plot the function and all primary central sample points."""
    ...
    maximum_distance = max(abs(offset) for offset in primary.offsets)
    required_span = maximum_distance * primary.step_size
    half_width = max(
        5.0 * max(required_span, primary.step_size),
        1.0,
        0.25 * max(1.0, abs(float(result.x_value))),
    )
    x_min = float(result.x_value) - half_width
    x_max = float(result.x_value) + half_width
    ...
    axis.scatter(
        ...
        s=70,
        label="Central sample points",
        zorder=5,
    )

    if result.derivative_order == 1:
        f_x0 = evaluate_real_scalar(
            numeric_function,
            float(result.x_value),
            "f(x₀) for the tangent line",
        )
        tangent_values = f_x0 + (
            primary.derivative_approximation
            * (x_values - float(result.x_value))
        )
        axis.plot(
            x_values,
            tangent_values,
    ...
    axis.set_title(
        f"Central Difference — Derivative Order {result.derivative_order}"
    )
    axis.set_xlabel("x")
    axis.set_ylabel("f(x)")
...
def create_convergence_plot(result: CentralDifferenceResult) -> plt.Figure:
    ...
    axis.invert_xaxis()
    axis.set_title("Central Difference — Convergence Analysis")
...
def create_approximation_plot(result: CentralDifferenceResult) -> plt.Figure:
    """Plot derivative approximations against refinement level."""
    ...
    exact_value = result.primary_exact_derivative
    if exact_value is not None and math.isfinite(exact_value):
        axis.axhline(
            exact_value,
            linestyle="--",
            linewidth=1.5,
    ...
    axis.set_title("Central Difference — Approximation by Refinement Level")
    axis.set_xlabel("Refinement Level")
    axis.set_ylabel(f"Derivative of Order {result.derivative_order}")
    axis.grid(True, alpha=0.3)
    axis.legend()
    figure.tight_layout()
    return figure
...
def create_excel_report(result: CentralDifferenceResult) -> bytes:
    ...
    approximation_figure = create_approximation_plot(result)
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        pd.DataFrame(
            {"Embedded Charts": ["Function graph", "Approximation graph", "Convergence graph"]}
        ...
        approximation_buffer = BytesIO(approximation_png)
        approximation_buffer.seek(0)
        image_buffers.append(approximation_buffer)
        approximation_image = ExcelImage(approximation_buffer)
        approximation_image.width = 760
        approximation_image.height = 450
        charts_sheet.add_image(approximation_image, "A28")
        ...
        if convergence_png is not None:
            ...
            charts_sheet.add_image(convergence_image, "A53")
...
# Streamlit rendering
# =============================================================================
def render_final_result(result: CentralDifferenceResult) -> None:
...
def render_primary_calculation(result: CentralDifferenceResult) -> None:
    """Render the entered-h calculation and Taylor series table."""

    st.subheader("Primary Central Difference Calculation")
    st.dataframe(
    ...
    st.info(
        "The central formulas are obtained by combining Taylor expansions "
        "so unwanted derivative terms cancel. The table displays the "
        "expansions through f‴(x₀), as requested."
    )


def render_refinement_analysis(result: CentralDifferenceResult) -> None:
    ...
    if result.observed_order_latest is not None:
        st.markdown(
            "**Representative observed convergence order:** "
...
def render_function_graph(result: CentralDifferenceResult) -> None:
...
def render_excel_download(result: CentralDifferenceResult) -> None:
    ...
    cached_signature = st.session_state.get(
        "central_difference_excel_signature"
    )
    ...
    if cached_signature != report_signature:
        try:
            ...
            return
        st.session_state.central_difference_excel_report = report_bytes
        st.session_state.central_difference_excel_signature = report_signature

    report_bytes = st.session_state.get("central_difference_excel_report")
    ...
    st.download_button(
        ...
        data=report_bytes,
        file_name=f"central_difference_report_{timestamp}.xlsx",
        mime=EXCEL_MIME_TYPE,
        use_container_width=True,
        key="central_difference_download_button",
    )
...
def render_page() -> None:
    """Render the complete Central Difference Streamlit solver page."""

    st.set_page_config(
        page_title="Central Difference Solver | Numerical Methods",
        page_icon="📈",
        layout="wide",
    )
    load_css()
    ...
    st.html(
        ...
        <section class="solver-hero">
            <div>
                ...
                <h1>Central Difference Solver</h1>
                <p>
                    Enter a function, evaluation point, and step size to compute
                    first-, second-, or third-order derivatives using standard
                    central finite-difference formulas. Review the complete
                    calculation, Taylor expansion, error analysis, graphs, and
                    Excel report.
                </p>

                <div class="method-actions">
                    <a href="/Central_Difference" target="_self"
                       class="btn-outline-ui">Review Lesson →</a>
                    <a href="/Central_Difference_Quiz" target="_self"
    ...
    with main_area:
        ...
        with conditions_column:
            with st.container(border=True):
                ...
                st.markdown(
                    ...
                    - The function must be finite at every required point on
                      both sides of **x₀**.
                    - The selected formula may require points from
                      **x₀ − 3h** through **x₀ + 3h**.
                    - Central formulas use symmetric Taylor expansions and
                      support derivatives through the third order.
                    - An extremely small **h** may increase cancellation and
                      round-off error.
                    """
                )
        ...
        with input_column:
            with st.container(border=True):
                ...
                function_text = st.text_input(
                    ...
                    key="central_difference_function",
                )
                ...
                with value_columns[0]:
                    ...
                    x_value = st.number_input(
                        ...
                        format="%.12g",
                        label_visibility="collapsed",
                        key="central_difference_x_value",
                    )
                ...
                with value_columns[1]:
                    ...
                    step_size = st.number_input(
                        ...
                        format="%.12g",
                        label_visibility="collapsed",
                        key="central_difference_step_size",
                    )
                ...
                with selector_columns[0]:
                    ...
                    derivative_name = st.selectbox(
                        ...
                        index=0,
                        label_visibility="collapsed",
                        key="central_difference_derivative_order",
                    )

                with selector_columns[1]:
                    st.markdown(
                        '<div class="input-label-ui">Central formula</div>',
                        unsafe_allow_html=True,
                    )
                    accuracy_name = st.selectbox(
                        ...
                        index=0,
                        label_visibility="collapsed",
                        key="central_difference_accuracy",
                    )
                ...
                refinement_levels = st.slider(
                    ...
                    step=1,
                    label_visibility="collapsed",
                    key="central_difference_refinement_levels",
                )

                st.caption(
                    "The primary result uses the entered h. Additional levels "
                    "use h/2, h/4, and so on for convergence analysis. "
                ...
                solve_button_clicked = st.button(
                    "Solve",
                    use_container_width=True,
                    key="central_difference_solve_button",
                )
        ...
        with result_column:
            with st.container(border=True):
                ...
                stored_result = st.session_state.get(
                    "central_difference_result"
                )
                if stored_result is None:
                    st.info(
                        "Enter the function and parameters, then click Solve."
                    )
                elif (
                    stored_result.input_signature
                    != current_input_signature
                ):
                    st.info(
                        "The function or numerical parameters changed. "
        ...
        if solve_button_clicked:
            st.session_state.central_difference_result = (
            ...
            st.session_state.pop(
                "central_difference_excel_report",
                None,
            )
            st.session_state.pop(
                "central_difference_excel_signature",
        ...
        active_result = st.session_state.get("central_difference_result")
        if (
            ...
            with navigation_left_column:
                if st.button(
                    "Review Central Difference Lesson",
                    use_container_width=True,
                    key="review_central_difference_lesson",
                ):
                    st.switch_page("pages/Central_Difference.py")

            with navigation_right_column:
                if st.button(
                    "Back to Solver Menu",
                    use_container_width=True,
                    key="back_to_solver_menu_central_difference",
                ):
    ...
    st.html(
        ...
        <footer class="footer-ui">
            ...
            <div>Numerical Differentiation • Central Difference</div>
        </footer>
        """
    )
