...
FORMULA_OPTIONS = {
    "Two-point central difference — second-order O(h²)": "two_point",
    "Five-point central difference — fourth-order O(h⁴)": "five_point",
}
...
class DifferenceEvaluation:
    """One central-difference calculation at a specific step size."""

    level: int
    step_size: float
    x_value: float
    x_minus_2h: float | None
    f_x_minus_2h: float | None
    x_minus_h: float
    f_x_minus_h: float
    f_x: float
    x_plus_h: float
...
# =============================================================================
# Numerical Central Difference algorithm
# =============================================================================
def formula_metadata(formula_key: str) -> tuple[str, int]:
    """Return the display formula and theoretical truncation-error order."""
    ...
    if formula_key == "five_point":
        return (
            "f′(x₀) ≈ [f(x₀−2h) − 8f(x₀−h) + 8f(x₀+h) − f(x₀+2h)] / (12h)",
            4,
        )
    raise ValueError("Unsupported central-difference formula.")
...
def calculate_central_difference(
    ...
    """Calculate one central-difference derivative approximation manually."""

    x_minus_h = x_value - step_size
    x_plus_h = x_value + step_size
    if not all(math.isfinite(value) for value in (x_minus_h, x_plus_h)):
    ...
    if formula_key == "two_point":
        ...
        substitution = (
            f"[{format_number(f_x_plus_h, 10)} − "
            f"{format_number(f_x_minus_h, 10)}] / "
            f"[2({format_number(step_size, 10)})]"
        )
    elif formula_key == "five_point":
        x_minus_2h = x_value - 2.0 * step_size
        x_plus_2h = x_value + 2.0 * step_size
        if not all(math.isfinite(value) for value in (x_minus_2h, x_plus_2h)):
            raise ValueError(
                "x₀ ± 2h is not finite. Reduce the magnitude of x₀ or h."
            )
        f_x_minus_2h = evaluate_real_scalar(
            numeric_function,
            x_minus_2h,
            "f(x₀ − 2h)",
        )
        ...
        denominator = 12.0 * step_size
        derivative = numerator / denominator
        substitution = (
            f"[{format_number(f_x_minus_2h, 10)} − "
            f"8({format_number(f_x_minus_h, 10)}) + "
            f"8({format_number(f_x_plus_h, 10)}) − "
            f"{format_number(f_x_plus_2h, 10)}] / "
            f"[12({format_number(step_size, 10)})]"
        )
    else:
        raise ValueError("Unsupported central-difference formula.")

    if not all(math.isfinite(float(value)) for value in (numerator, denominator, derivative)):
        raise ValueError("The central-difference calculation produced a non-finite value.")

    return {
        "x_minus_2h": x_minus_2h,
        "f_x_minus_2h": f_x_minus_2h,
        "x_minus_h": x_minus_h,
...
def solve_central_difference(
    ...
    """Validate inputs and perform the complete Central Difference workflow."""
    ...
    try:
        ...
        if formula_key is None:
            raise ValueError("Select a valid Central Difference formula.")
        formula_text, theoretical_order = formula_metadata(formula_key)
        ...
        for level in range(refinement_levels):
            ...
            successive_difference = (
                abs(approximation - approximations[-1]) if approximations else None
            )
            ...
            evaluations.append(
                DifferenceEvaluation(
                    ...
                    x_minus_2h=(
                        None
                        if calculation["x_minus_2h"] is None
                        else float(calculation["x_minus_2h"])
                    ),
                    f_x_minus_2h=(
                        None
                        if calculation["f_x_minus_2h"] is None
                        else float(calculation["f_x_minus_2h"])
                    ),
                    x_minus_h=float(calculation["x_minus_h"]),
                    f_x_minus_h=float(calculation["f_x_minus_h"]),
        ...
        if abs(primary.f_x_plus_h - primary.f_x_minus_h) <= ZERO_TOLERANCE:
            warnings.append(
                "The sampled values f(x₀+h) and f(x₀−h) are numerically identical. "
                "The derivative approximation may be zero because of symmetry, "
                "local behavior, or floating-point cancellation."
            )
        ...
        if finest.step_size < np.cbrt(np.finfo(float).eps) * max(
            1.0,
            abs(x_value),
        ):
            warnings.append(
                "The finest step is very small relative to x₀. Central differences "
        ...
        return CentralDifferenceResult(
            ...
            stopping_reason=(
                "The requested Central Difference approximation and step-size "
...
# DataFrame builders
# =============================================================================
def create_evaluation_dataframe(result: CentralDifferenceResult) -> pd.DataFrame:
        ...
        rows.append(
            {
                "Level": item.level,
                "h": item.step_size,
                "x0 - 2h": item.x_minus_2h,
                "f(x0 - 2h)": item.f_x_minus_2h,
                "x0 - h": item.x_minus_h,
...
def create_primary_calculation_dataframe(
    ...
    if result.formula_key == "five_point":
        rows.extend(
            [
                ("x₀ − 2h", item.x_minus_2h),
                ("f(x₀ − 2h)", item.f_x_minus_2h),
            ]
        )
    rows.extend(
        [
            ("x₀ − h", item.x_minus_h),
            ("f(x₀ − h)", item.f_x_minus_h),
            ("f(x₀)", item.f_x),
            ("x₀ + h", item.x_plus_h),
...
def create_error_dataframe(result: CentralDifferenceResult) -> pd.DataFrame:
...
def create_function_plot(result: CentralDifferenceResult) -> plt.Figure:
    """Plot the function, central sample points, and approximate tangent line."""
    ...
    axis.scatter(
        [primary.x_minus_h, primary.x_plus_h],
        [primary.f_x_minus_h, primary.f_x_plus_h],
        s=70,
        label="Central points x₀ ± h",
    )
    if (
        primary.x_minus_2h is not None
        and primary.f_x_minus_2h is not None
        and primary.x_plus_2h is not None
        and primary.f_x_plus_2h is not None
    ):
        axis.scatter(
            [primary.x_minus_2h, primary.x_plus_2h],
            [primary.f_x_minus_2h, primary.f_x_plus_2h],
            s=70,
            label="Outer central points x₀ ± 2h",
        )

    axis.set_title("Central Difference — Function and Approximate Tangent")
    axis.set_xlabel("x")
    axis.set_ylabel("f(x)")
...
def render_excel_download(result: CentralDifferenceResult) -> None:
    ...
    cached_signature = st.session_state.get("central_difference_excel_signature")
        ...
        <section class="solver-hero">
            <div>
                ...
                <p>
                    ...
                    the first derivative using standard central finite-difference
    ...
    with main_area:
        ...
        with conditions_column:
            with st.container(border=True):
                ...
                - The function must be finite on both sides of **x₀**.
                - The two-point formula requires **x₀ ± h**; the five-point formula also requires **x₀ ± 2h**.
                - Central differences are usually more accurate than one-sided differences for the same step size.
                - An extremely small **h** may cause cancellation and rounding error.
                    """
                )
        ...
        with input_column:
            with st.container(border=True):
                ...
                formula_name = st.selectbox(
                    ...
                    index=0,
                    label_visibility="collapsed",
                    key="central_difference_formula",
                )
                ...
                st.caption(
                    ...
                    "h/2, h/4, and so on for convergence analysis. The function "
                    "must be real and defined at all required points on both sides of x₀."
                )
        ...
        with result_column:
            with st.container(border=True):
                ...
                stored_result = st.session_state.get("central_difference_result")
        ...
        if solve_button_clicked:
            st.session_state.central_difference_result = solve_central_difference(
                function_text=function_text,
                raw_x_value=x_value,
            ...
            st.session_state.pop("central_difference_excel_report", None)
            st.session_state.pop("central_difference_excel_signature", None)
            st.rerun()
