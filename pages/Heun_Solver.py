...
class HeunIteration:
    """One standard Heun predictor-corrector step."""
    ...
    step_size: float
    k1: float
    predictor_increment: float
    predictor_x: float
    predictor_y: float
    k2: float
    average_slope: float
    corrected_increment: float
    x_next: float
    y_next: float
    step_change: float
...
class ConvergenceRecord:
    """One grid-refinement result for The Heun method."""
...
def run_heun_grid(
...
) -> tuple[float, tuple[HeunIteration, ...]]:
    """Run the standard Heun Method on a uniform grid.

    For each step:

        k1 = f(x_n, y_n)
        y_predictor = y_n + h*k1
        k2 = f(x_n + h, y_predictor)
    ...
    for iteration in range(1, steps + 1):
        ...
        predictor_increment = step_size * k1
        predictor_x = x0 + iteration * step_size
        predictor_y = current_y + predictor_increment

        if not all(
            math.isfinite(value)
            for value in (predictor_increment, predictor_x, predictor_y)
        ):
            raise ValueError(
                "Heun's Method produced a non-finite Euler predictor "
                f"at iteration {iteration}."
            )

        k2 = evaluate_ode(ode_function, predictor_x, predictor_y)
        average_slope = 0.5 * (k1 + k2)
        corrected_increment = step_size * average_slope
        next_x = predictor_x
        next_y = current_y + corrected_increment

        if not all(
            math.isfinite(value)
            for value in (average_slope, corrected_increment, next_x, next_y)
        ):
            raise ValueError(
                "Heun's Method produced a non-finite corrected approximation "
                f"at iteration {iteration}."
            )
        ...
        if store_history:
            history.append(
                HeunIteration(
                    ...
                    k1=float(k1),
                    predictor_increment=float(predictor_increment),
                    predictor_x=float(predictor_x),
                    predictor_y=float(predictor_y),
                    k2=float(k2),
                    average_slope=float(average_slope),
                    corrected_increment=float(corrected_increment),
                    x_next=float(next_x),
                    y_next=float(next_y),
                    ...
                    operation=(
                        ...
                        f"predictor = {current_y:.12g} + "
                        f"({step_size:.12g})({k1:.12g}) = {predictor_y:.12g}; "
                        f"k2 = f({predictor_x:.12g}, {predictor_y:.12g}) = {k2:.12g}; "
                        f"y_{iteration} = {current_y:.12g} + "
                        f"({step_size:.12g}/2)({k1:.12g} + {k2:.12g}) "
                        f"= {next_y:.12g}"
                    ),
                    status="Completed",
                )
...
def build_convergence_analysis(
    ...
    """Refine the grid and estimate the expected second-order convergence."""

    raw_records: list[dict[str, float | int | None]] = []

    for level in range(1, levels + 1):
        steps = base_steps * (2 ** (level - 1))
        final_y, _ = run_heun_grid(
...
def error_result(message: str, input_signature: str = "") -> HeunResult:
...
def solve_heun_method(
    ...
    try:
        ...
        final_y, iterations = run_heun_grid(
            ode_function=ode_function,
            x0=x0,
            y0=y0,
            x_end=x_end,
        ...
        if any(abs(record.y_next) >= VALUE_MAGNITUDE_WARNING for record in iterations):
            warnings.append(
                "The numerical solution reached a very large magnitude. The Heun "
                "method may be unstable for this step size or differential equation."
            )

        if latest_observed_order is not None and latest_observed_order < 1.2:
            warnings.append(
                "The observed refinement behavior is weaker than the expected "
                "second-order convergence. Reduce the step size and inspect possible "
        ...
        return HeunResult(
            ...
            stopping_reason=(
                "The requested number of uniform Heun steps was completed and "
...
# DataFrame builders
# =============================================================================
def iterations_dataframe(result: HeunResult) -> pd.DataFrame:
    """Return the complete Heun predictor-corrector iteration table."""
        ...
        rows.append(
            {
                ...
                "y_n": record.y_n,
                "h": record.step_size,
                "k1 = f(x_n, y_n)": record.k1,
                "Predictor Increment h*k1": record.predictor_increment,
                "x Predictor": record.predictor_x,
                "y Predictor": record.predictor_y,
                "k2 = f(x_(n+1), y Predictor)": record.k2,
                "Average Slope (k1+k2)/2": record.average_slope,
                "Corrected Increment": record.corrected_increment,
...
def solution_values_dataframe(result: HeunResult) -> pd.DataFrame:
    ...
    rows = [
        {
            "Point": 0,
            "x": result.x0,
            "Corrected Heun Approximation": result.y0,
            "Exact Solution": (
            ...
            "Absolute Error": 0.0 if result.exact_solution_expression is not None else None,
        }
    ]
        ...
        rows.append(
            {
                ...
                "x": record.x_next,
                "Corrected Heun Approximation": record.y_next,
...
def error_analysis_dataframe(result: HeunResult) -> pd.DataFrame:
    ...
    if result.exact_solution_expression is None:
        return pd.DataFrame(
            {
                "Message": [
                    "No exact solution was supplied; true global error is unavailable."
                ]
            }
        )
...
def convergence_dataframe(result: HeunResult) -> pd.DataFrame:
...
def method_formula_dataframe(result: HeunResult) -> pd.DataFrame:
    """Return Heun's Method formulas for the Excel report."""
...
# Scientific plots
# =============================================================================
def create_solution_figure(result: HeunResult) -> Figure:
    """Plot the Heun solution and optional exact solution."""
    ...
    axis.plot(
        solution_df["x"],
        solution_df["Corrected Heun Approximation"],
        marker="o",
        linewidth=2,
        label="Corrected Heun Approximation",
    )
    ...
    predictor_x = [record.predictor_x for record in result.iterations]
    predictor_y = [record.predictor_y for record in result.iterations]
    axis.scatter(
        predictor_x,
        predictor_y,
        s=35,
        marker="x",
        label="Euler Predictor Points",
        zorder=4,
    )
    axis.scatter(
        [result.x0],
    ...
    axis.scatter(
        ...
        s=110,
        marker="*",
        label=f"Final Corrected Value = {result.final_y:.6g}",
        zorder=6,
    )
    axis.axhline(0.0, linewidth=1)
    axis.axvline(0.0, linewidth=1)
    axis.set_title("Heun Method Numerical Solution")
    axis.set_xlabel("x")
    axis.set_ylabel("y")
    axis.grid(True)
    axis.legend()
...
def create_error_figure(result: HeunResult) -> Figure | None:
    ...
    axis.set_title("Heun Method Global Error")
    axis.set_xlabel("x")
...
def create_convergence_figure(result: HeunResult) -> Figure:
    ...
    if np.any(valid):
        ...
        reference_line = reference_metric[-1] * (reference_h / reference_h[-1]) ** 2
        axis.loglog(
            reference_h,
            reference_line,
            linestyle="--",
    ...
    axis.set_title("Heun Method Grid-Refinement Convergence")
    axis.set_xlabel("|h|")
...
def generate_excel_report(result: HeunResult) -> bytes:
    """Create a formatted in-memory XLSX report for a successful result."""

    if not result.success:
        raise ValueError("Only successful Heun results can be exported.")
    ...
    summary_rows = [
        ...
        ("Final Corrected Heun Approximation", result.final_y),
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if result.iterations:
            chart = LineChart()
            chart.title = "Heun Numerical Solution"
            chart.y_axis.title = "y"
            chart.x_axis.title = "x"
            max_row = len(result.iterations) + 1
            x_reference = Reference(iteration_sheet, min_col=10, min_row=2, max_row=max_row)
            y_reference = Reference(iteration_sheet, min_col=11, min_row=1, max_row=max_row)
...
def render_final_result(result: HeunResult) -> None:
    ...
    metric_columns[1].metric("Corrected Heun Approximation", format_number(result.final_y))

    st.markdown(f"**Differential Equation:** `y' = {result.ode_text}`")
    st.markdown(
        f"**Initial Condition:** `y({format_number(result.x0)}) = "
    ...
    if result.exact_final_y is not None:
        ...
        st.markdown(
            f"**Absolute Final Error:** {format_number(result.absolute_final_error)}"
        )
        st.markdown(
            f"**Relative Final Error:** {format_number(result.relative_final_error)}"
        )
...
def render_method_summary(result: HeunResult) -> None:
    """Render the Heun formulas and problem summary."""
    ...
    with column_1:
        ...
        st.latex(r"y_{n+1}^{(p)}=y_n+h k_1")
    with column_2:
        st.latex(r"k_2=f(x_n+h,y_{n+1}^{(p)})")
        st.latex(r"\bar{k}=\frac{k_1+k_2}{2}")
        st.latex(r"y_{n+1}=y_n+\frac{h}{2}(k_1+k_2)")
    ...
    st.dataframe(
        round_numeric_dataframe(summary_df), use_container_width=True, hide_index=True
    )


def render_iteration_table(result: HeunResult) -> None:
    """Render the full Heun step table and detailed operations."""
    ...
    st.caption(
        "The predictor is an Euler estimate used only to evaluate the second "
        "slope. The accepted value is the corrected Heun approximation. Step "
        "Change is not the true global error unless an exact solution is supplied."
    )

    with st.expander("Detailed Heun Predictor-Corrector Operations"):
        for record in result.iterations:
            st.markdown(f"**Iteration {record.iteration}:** `{record.operation}`")
            st.caption(
                f"k1 = {record.k1:.12g}; Euler predictor = "
                f"({record.predictor_x:.12g}, {record.predictor_y:.12g}); "
                f"k2 = {record.k2:.12g}; average slope = "
                f"{record.average_slope:.12g}; corrected increment = "
                f"{record.corrected_increment:.12g}."
            )


def render_solution_graph(result: HeunResult) -> None:
...
def render_error_analysis(result: HeunResult) -> None:
...
def render_convergence_analysis(result: HeunResult) -> None:
    """Render grid-refinement convergence analysis."""

    st.subheader("Convergence Analysis")
    st.markdown(
        "The explicit Heun method has **second-order global convergence** "
        "under the usual smoothness and stability conditions. Halving the step "
        "size should reduce the global error by approximately a factor of four "
...
def render_excel_download(result: HeunResult) -> None:
    ...
    if (
        st.session_state.get("heun_excel_signature") != report_signature
        or "heun_excel_report" not in st.session_state
    ):
        try:
            st.session_state.heun_excel_report = generate_excel_report(result)
            st.session_state.heun_excel_signature = report_signature
        except (ValueError, OSError, TypeError, ArithmeticError) as error:
            st.error(f"The Excel report could not be generated: {error}")
            return

    report_bytes = st.session_state.get("heun_excel_report")
    if not report_bytes:
        st.error("The Excel report is unavailable.")
        return

    timestamp = result.execution_datetime.strftime("%Y%m%d_%H%M%S")
    filename = f"heun_method_report_{timestamp}.xlsx"
    st.download_button(
    ...
    st.html(
        ...
        <section class="solver-hero">
            <div>
                ...
                <p>
                    Solve a first-order initial-value problem using the explicit
                    Heun's Method, a second-order predictor-corrector Runge–Kutta scheme. Review every
                    first slope, Euler predictor, second slope, corrected update,
                    optional exact-solution error, convergence result, graph, and report.
                </p>
    ...
    with main_area:
        ...
        with conditions_column:
            with st.container(border=True):
                ...
                - Heun’s Method uses an Euler predictor and averages the beginning and predicted slopes.
                - The optional exact solution must satisfy the supplied initial condition.
                - The equation must remain finite at both the current and predicted points.
                    """
                )
            ...
            with st.container(border=True):
                ...
                with first_row[2]:
                    ...
                    x_end = st.number_input(
                        ...
                        label_visibility="collapsed", key="predictor_x_end"
                    )
                ...
                with second_row[1]:
                    ...
                    convergence_levels = st.number_input(
                        ...
                        label_visibility="collapsed", key="heun_convergence_levels"
                    )
                ...
                exact_solution_text = st.text_input(
                    ...
                    label_visibility="collapsed", key="heun_exact_solution"
                )
                ...
                solve_clicked = st.button(
                    "Solve with Heun Method", type="primary", use_container_width=True,
                ...
                if solve_clicked:
                    ...
                    st.session_state.heun_result_signature = current_signature
                    st.session_state.pop("heun_excel_report", None)
                    st.session_state.pop("heun_excel_signature", None)
        ...
        with result_column:
            with st.container(border=True):
                ...
                saved_result = st.session_state.get("heun_result")
                saved_signature = st.session_state.get("heun_result_signature")
    
                if saved_result is None:
                    st.info("Enter the IVP data and select Solve with Heun Method.")
                elif saved_signature != current_signature:
                    st.warning(
                        "The inputs have changed. Select Solve with Heun Method "
