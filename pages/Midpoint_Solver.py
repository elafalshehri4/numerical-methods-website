...
class MidpointIteration:
    """One explicit Midpoint (second-order Runge-Kutta) step."""
...
def run_midpoint_grid(
...
) -> tuple[float, tuple[MidpointIteration, ...]]:
    """Run the standard explicit Midpoint Method on a uniform grid.

    The method is the second-order Runge-Kutta midpoint scheme:
    ...
    for iteration in range(1, steps + 1):
        ...
        midpoint_y = current_y + 0.5 * step_size * k1
        if not math.isfinite(midpoint_x) or not math.isfinite(midpoint_y):
            raise ValueError(
                "The Midpoint method produced a non-finite midpoint "
                f"at iteration {iteration}."
            )

        k2 = evaluate_ode(ode_function, midpoint_x, midpoint_y)
        midpoint_increment = step_size * k2
        next_x = x0 + iteration * step_size
        next_y = current_y + midpoint_increment

        if not math.isfinite(next_x) or not math.isfinite(next_y):
            raise ValueError(
                "The Midpoint method produced a non-finite approximation "
                f"at iteration {iteration}."
            )
        ...
        if store_history:
            history.append(
                MidpointIteration(
                    ...
                    k2=float(k2),
                    midpoint_increment=float(midpoint_increment),
                    x_next=float(next_x),
                    y_next=float(next_y),
                    ...
                    operation=(
                        ...
                        f"k2 = f({midpoint_x:.12g}, {midpoint_y:.12g}) = {k2:.12g}; "
                        f"y_{iteration} = {current_y:.12g} + "
                        f"({step_size:.12g})({k2:.12g}) = {next_y:.12g}"
                    ),
                    status="Completed",
                )
            )
...
def error_result(message: str, input_signature: str = "") -> MidpointResult:
...
def solve_midpoint_method(
    ...
    try:
        ...
        final_y, iterations = run_midpoint_grid(
            ode_function=ode_function,
            x0=x0,
            y0=y0,
            x_end=x_end,
        ...
        if any(abs(record.y_next) >= VALUE_MAGNITUDE_WARNING for record in iterations):
            warnings.append(
                "The numerical solution reached a very large magnitude. The Midpoint "
        ...
        return MidpointResult(
            ...
            stopping_reason=(
                "The requested number of uniform Midpoint steps was completed and "
...
# DataFrame builders
# =============================================================================
def iterations_dataframe(result: MidpointResult) -> pd.DataFrame:
    """Return the complete Midpoint iteration table."""
        ...
        rows.append(
            {
                ...
                "x_mid": record.midpoint_x,
                "y_mid": record.midpoint_y,
                "k2 = f(x_mid, y_mid)": record.k2,
                "Midpoint Increment h*k2": record.midpoint_increment,
...
def solution_values_dataframe(result: MidpointResult) -> pd.DataFrame:
    ...
    rows = [
        {
            "Point": 0,
            "x": result.x0,
            "Midpoint Approximation": result.y0,
            "Exact Solution": (
        ...
        rows.append(
            {
                ...
                "x": record.x_next,
                "Midpoint Approximation": record.y_next,
...
def error_analysis_dataframe(result: MidpointResult) -> pd.DataFrame:
...
def convergence_dataframe(result: MidpointResult) -> pd.DataFrame:
...
def method_formula_dataframe(result: MidpointResult) -> pd.DataFrame:
    """Return the method formulas for the Excel report."""
    ...
    return pd.DataFrame(
        {
            ...
            "Formula": [
                ...
                "h = (x_end - x0) / N",
                "k1 = f(x_n, y_n)",
                "x_mid = x_n + h/2; y_mid = y_n + h*k1/2",
                "k2 = f(x_mid, y_mid)",
                "y_(n+1) = y_n + h*k2",
                "O(h^2)",
                "O(h^3) per step",
...
# Scientific plots
# =============================================================================
def create_solution_figure(result: MidpointResult) -> Figure:
    """Plot the Midpoint solution and optional exact solution."""
    ...
    axis.plot(
        solution_df["x"],
        solution_df["Midpoint Approximation"],
        marker="o",
        linewidth=2,
    ...
    midpoint_x = [record.midpoint_x for record in result.iterations]
    midpoint_y = [record.midpoint_y for record in result.iterations]
    axis.scatter(
        midpoint_x,
        midpoint_y,
        s=35,
        marker="x",
        label="Stage Midpoints",
        zorder=4,
    )
    axis.scatter(
        [result.x0],
    ...
    axis.scatter(
        ...
        s=110,
        marker="*",
        label=f"Final Midpoint Value = {result.final_y:.6g}",
        zorder=6,
    )
    axis.axhline(0.0, linewidth=1)
    axis.axvline(0.0, linewidth=1)
    axis.set_title("Midpoint Method Numerical Solution")
    axis.set_xlabel("x")
    axis.set_ylabel("y")
    axis.grid(True)
    axis.legend()
...
def create_error_figure(result: MidpointResult) -> Figure | None:
    ...
    axis.set_title("Midpoint Method Global Error")
    axis.set_xlabel("x")
...
def create_convergence_figure(result: MidpointResult) -> Figure:
    ...
    axis.set_title("Midpoint Method Grid-Refinement Convergence")
    axis.set_xlabel("|h|")
...
def generate_excel_report(result: MidpointResult) -> bytes:
    """Create a formatted in-memory XLSX report for a successful result."""

    if not result.success:
        raise ValueError("Only successful Midpoint results can be exported.")
    ...
    summary_rows = [
        ...
        ("Final Midpoint Approximation", result.final_y),
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if result.iterations:
            chart = LineChart()
            chart.title = "Midpoint Numerical Solution"
...
def render_final_result(result: MidpointResult) -> None:
    ...
    metric_columns[1].metric("Midpoint Approximation", format_number(result.final_y))
...
def render_method_summary(result: MidpointResult) -> None:
    """Render the formula and problem summary."""
    ...
    with column_1:
        ...
        st.latex(r"x_{n+\frac12}=x_n+\frac{h}{2}")
    with column_2:
        st.latex(r"y_{n+\frac12}=y_n+\frac{h}{2}k_1")
        st.latex(r"k_2=f(x_{n+\frac12},y_{n+\frac12})")
...
def render_iteration_table(result: MidpointResult) -> None:
    """Render the full Midpoint step table and detailed operations."""
    ...
    st.caption(
        "Step Change is |y_(n+1) - y_n|. It is not the true numerical error. "
        "True global error is shown only when an exact solution is supplied."
    )

    with st.expander("Detailed Midpoint Operations"):
        for record in result.iterations:
            st.markdown(f"**Iteration {record.iteration}:** `{record.operation}`")
            st.caption(
                f"k1 = {record.k1:.12g}; midpoint = "
                f"({record.midpoint_x:.12g}, {record.midpoint_y:.12g}); "
                f"k2 = {record.k2:.12g}; h·k2 = {record.midpoint_increment:.12g}."
            )


def render_solution_graph(result: MidpointResult) -> None:
...
def render_error_analysis(result: MidpointResult) -> None:
...
def render_convergence_analysis(result: MidpointResult) -> None:
    """Render grid-refinement convergence analysis."""

    st.subheader("Convergence Analysis")
    st.markdown(
        "The explicit Midpoint method has **second-order global convergence** "
...
def render_excel_download(result: MidpointResult) -> None:
    ...
    if (
        st.session_state.get("midpoint_excel_signature") != report_signature
        or "midpoint_excel_report" not in st.session_state
    ):
        try:
            st.session_state.midpoint_excel_report = generate_excel_report(result)
            st.session_state.midpoint_excel_signature = report_signature
        except (ValueError, OSError, TypeError, ArithmeticError) as error:
            st.error(f"The Excel report could not be generated: {error}")
            return

    report_bytes = st.session_state.get("midpoint_excel_report")
    if not report_bytes:
        st.error("The Excel report is unavailable.")
        return

    timestamp = result.execution_datetime.strftime("%Y%m%d_%H%M%S")
    filename = f"midpoint_method_report_{timestamp}.xlsx"
    st.download_button(
        ...
        <section class="solver-hero">
            <div>
                ...
                <p>
                    ...
                    Midpoint Method, a second-order Runge–Kutta scheme. Review every
                    first-stage slope, midpoint estimate, second-stage slope, update,
    ...
    with main_area:
        ...
        with conditions_column:
            with st.container(border=True):
                ...
                - The explicit Midpoint Method evaluates the slope at the middle of each step and is second-order accurate.
                - The optional exact solution must satisfy the supplied initial condition.
                - The differential equation must remain finite at all midpoint stage values.
                    """
                )
            ...
            with st.container(border=True):
                ...
                with first_row[2]:
                    ...
                    x_end = st.number_input(
                        ...
                        label_visibility="collapsed", key="midpoint_x_end"
                    )
                ...
                with second_row[1]:
                    ...
                    convergence_levels = st.number_input(
                        ...
                        label_visibility="collapsed", key="midpoint_convergence_levels"
                    )
                ...
                exact_solution_text = st.text_input(
                    ...
                    label_visibility="collapsed", key="midpoint_exact_solution"
                )
                ...
                solve_clicked = st.button(
                    "Solve with Midpoint Method", type="primary", use_container_width=True,
                ...
                if solve_clicked:
                    ...
                    st.session_state.midpoint_result_signature = current_signature
                    st.session_state.pop("midpoint_excel_report", None)
                    st.session_state.pop("midpoint_excel_signature", None)
        ...
        with result_column:
            with st.container(border=True):
                ...
                saved_result = st.session_state.get("midpoint_result")
                saved_signature = st.session_state.get("midpoint_result_signature")
    
                if saved_result is None:
                    st.info("Enter the IVP data and select Solve with Midpoint Method.")
                elif saved_signature != current_signature:
                    st.warning(
                        "The inputs have changed. Select Solve with Midpoint Method "
