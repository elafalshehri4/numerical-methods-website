...
# Constants
# =============================================================================
METHOD_NAME = "Explicit Midpoint Method (RK2)"
DISPLAY_DECIMALS = 3
DEFAULT_ORDER = 1
DEFAULT_ODE = "x + y"
DEFAULT_X0 = 0.0
...
class MidpointIteration:
    """One Midpoint RK2 step for an ODE of order 1, 2, or 3."""
    ...
    midpoint_x: float
    midpoint_y: float
    midpoint_yp: float | None
    midpoint_ypp: float | None

    k2_y: float
    k2_yp: float | None
    k2_ypp: float | None

    midpoint_increment_y: float
    midpoint_increment_yp: float | None
    midpoint_increment_ypp: float | None
...
class ConvergenceRecord:
    """One grid-refinement result for the Midpoint method."""
...
class MidpointResult:
    """Complete Midpoint result shared by Streamlit and Excel renderers."""
    ...
    iterations: tuple[MidpointIteration, ...]
...
def component_operation_text(
    ...
    """Build complete midpoint-stage and accepted-update descriptions."""

    midpoint = (
        f"{label}_mid = {current_value:.12g} + "
        f"({step_size:.12g}/2)({k1_value:.12g}) = "
        f"{midpoint_value:.12g}"
    )
    update = (
        f"{label}_{iteration} = {current_value:.12g} + "
        f"({step_size:.12g})({k2_value:.12g}) = "
        f"{next_value:.12g}"
    )
    return midpoint, update
...
def run_midpoint_grid(
    ...
    tuple[float, float | None, float | None],
    tuple[MidpointIteration, ...],
]:
    """Run the explicit Midpoint RK2 method on the first-order system.
    ...
    For second- and third-order equations, all state components are advanced
    ...
    exact_functions = build_exact_functions(exact_expression)
    history: list[MidpointIteration] = []
    ...
    for iteration in range(1, steps + 1):
        ...
        midpoint_x = current_x + 0.5 * step_size
        midpoint_state = current_state + 0.5 * step_size * k1

        if not np.all(np.isfinite(midpoint_state)):
            raise ValueError(
                f"The Midpoint method produced a non-finite midpoint state "
                f"at iteration {iteration}."
            )

        midpoint_y, midpoint_yp, midpoint_ypp = unpack_state(
            ode_order,
            midpoint_state,
        )
        ...
        midpoint_increment = step_size * k2
        next_state = current_state + step_size * k2
        next_x = x0 + iteration * step_size

        if not np.all(np.isfinite(next_state)):
            raise ValueError(
                f"The Midpoint method produced a non-finite updated state "
                f"at iteration {iteration}."
            )

        next_y, next_yp, next_ypp = unpack_state(
            ode_order,
            next_state,
        )
        ...
        if exact_expression is not None:
            ...
            exact_midpoint_state = (
                exact_current_state + 0.5 * step_size * exact_k1
            )
            (
                exact_midpoint_y,
                exact_midpoint_yp,
            ...
            exact_one_step_state = (
                exact_current_state + step_size * exact_k2
            )
            local_truncation_errors = (
                exact_next_state - exact_one_step_state
            )

        midpoint_parts: list[str] = []
        update_parts: list[str] = []
        component_names = ["y", "yp", "ypp"]

        for component_index in range(ode_order):
            midpoint_text, update_text = component_operation_text(
                component_names[component_index],
                iteration,
                float(current_state[component_index]),
                step_size,
                float(k1[component_index]),
                float(k2[component_index]),
                float(midpoint_state[component_index]),
                float(next_state[component_index]),
            )
            midpoint_parts.append(midpoint_text)
        ...
        if store_history:
            history.append(
                MidpointIteration(
                    ...
                    midpoint_yp=midpoint_yp,
                    midpoint_ypp=midpoint_ypp,

                    k2_y=float(k2[0]),
                    k2_yp=float(k2[1]) if ode_order >= 2 else None,
                    k2_ypp=float(k2[2]) if ode_order >= 3 else None,

                    midpoint_increment_y=float(midpoint_increment[0]),
                    midpoint_increment_yp=(
                        float(midpoint_increment[1])
                        if ode_order >= 2
                        else None
                    ),
                    midpoint_increment_ypp=(
                        float(midpoint_increment[2])
                        if ode_order >= 3
                        else None
                    ),
                    ...
                    relative_error_yp_percent=(
                        relative_errors_percent[1]
                        if ode_order >= 2
                        else None
                    ),
                    relative_error_ypp_percent=(
                        relative_errors_percent[2]
                        if ode_order >= 3
                        else None
                    ),
                    ...
                    midpoint_operation="; ".join(midpoint_parts),
    ...
    final_y, final_yp, final_ypp = unpack_state(
        ode_order,
        current_state,
    )
...
def build_convergence_analysis(
    ...
    """Refine the grid and estimate the Midpoint method's second-order convergence."""
    ...
    for level in range(1, levels + 1):
        ...
        final_state_tuple, _ = run_midpoint_grid(
...
def solve_midpoint_method(
    ...
    try:
        ...
        x0 = validate_finite_real(
            x0_input,
            "Initial x value x0",
        )
        y0 = validate_finite_real(
            y0_input,
            "Initial value y(x0)",
        )
        x_end = validate_finite_real(
            x_end_input,
            "Final x value",
        )
        ...
        try:
            ...
            raise ValueError(
                "Convergence levels must be an integer."
            ) from error

        if not MIN_STEPS <= steps <= MAX_STEPS:
            raise ValueError(
                f"The number of steps must be between "
        ...
        if not (
            ...
            raise ValueError(
                ...
                f"{MIN_CONVERGENCE_LEVELS} and "
                f"{MAX_CONVERGENCE_LEVELS}."
            )

        refined_steps = (
            steps * (2 ** (convergence_levels - 1))
        )
        ...
        if abs(x_end - x0) <= ZERO_TOLERANCE:
            raise ValueError(
                "The final x value must be different from x0."
            )
        ...
        exact_expression, _ = parse_exact_solution(
            exact_solution_text
        )
        ...
        final_state, iterations = run_midpoint_grid(
            ode_function=ode_function,
            ode_order=ode_order,
            x0=x0,
            y0=y0,
        ...
        if exact_expression is not None:
            exact_final_state = exact_state_at(
                exact_functions,
                ode_order,
                x_end,
            )
            ...
            signed_final_errors = (
                numerical_final_state - exact_final_state
            )
            absolute_final_errors = np.abs(
                signed_final_errors
            )

            exact_final_y = float(exact_final_state[0])
            signed_final_error_y = float(
                signed_final_errors[0]
            )
            absolute_final_error_y = float(
                absolute_final_errors[0]
            )
            relative_final_error_y_percent = (
                safe_relative_percent(
                    absolute_final_error_y,
                    exact_final_y,
                )
            )

            if ode_order >= 2:
                exact_final_yp = float(
                    exact_final_state[1]
                )
                signed_final_error_yp = float(
                    signed_final_errors[1]
                )
                absolute_final_error_yp = float(
                    absolute_final_errors[1]
                )
                relative_final_error_yp_percent = (
                    safe_relative_percent(
                        absolute_final_error_yp,
                        exact_final_yp,
                    )
                )

            if ode_order >= 3:
                exact_final_ypp = float(
                    exact_final_state[2]
                )
                signed_final_error_ypp = float(
                    signed_final_errors[2]
                )
                absolute_final_error_ypp = float(
                    absolute_final_errors[2]
                )
                relative_final_error_ypp_percent = (
                    safe_relative_percent(
                        absolute_final_error_ypp,
                        exact_final_ypp,
                    )
                )
            ...
            if y_errors.size:
                maximum_absolute_error_y = float(
                    np.max(np.abs(y_errors))
                )
                rmse_y = float(
                    np.sqrt(np.mean(y_errors**2))
                )
        ...
        midpoint_magnitudes: list[float] = []

        for record in iterations:
            state_magnitudes.append(abs(record.y_next))
            midpoint_magnitudes.append(
                abs(record.midpoint_y)
            )

            if record.yp_next is not None:
                state_magnitudes.append(
                    abs(record.yp_next)
                )
            ...
            if record.midpoint_yp is not None:
                midpoint_magnitudes.append(
                    abs(record.midpoint_yp)
                )
            if record.midpoint_ypp is not None:
                midpoint_magnitudes.append(
                    abs(record.midpoint_ypp)
                )

        if (
            state_magnitudes
            and max(state_magnitudes)
            >= VALUE_MAGNITUDE_WARNING
        ):
            warnings.append(
                "The updated numerical state reached a very "
                "large magnitude. the explicit Midpoint method may be "
                "unstable for this step size or problem."
            )

        if (
            midpoint_magnitudes
            and max(midpoint_magnitudes)
            >= VALUE_MAGNITUDE_WARNING
        ):
            warnings.append(
                "The half-step midpoint estimate reached a very large magnitude. "
                "Reduce the step size and inspect stability."
            )

        if (
            latest_observed_order is not None
            and latest_observed_order < 1.2
        ):
            warnings.append(
                "The observed convergence is weaker than the "
                "expected second-order behavior. Reduce the step "
                "size and inspect stability or nonsmooth behavior."
            )

        if exact_expression is None:
            warnings.append(
                "No exact solution was supplied. True global errors "
                "are unavailable; successive final-value differences "
        ...
        return MidpointResult(
            ...
            stopping_reason=(
                "The requested number of uniform explicit Midpoint RK2 steps "
                "was completed and the final grid point was reached."
            ),
            ...
            signed_final_error_y=(
                signed_final_error_y
            ),
            signed_final_error_yp=(
                signed_final_error_yp
            ),
            signed_final_error_ypp=(
                signed_final_error_ypp
            ),
            absolute_final_error_y=(
                absolute_final_error_y
...
def iterations_dataframe(
    ...
    """Return the complete Midpoint RK2 table."""
    ...
    for record in result.iterations:
        row: dict[str, Any] = {
            ...
            "h": record.step_size,
            "K1_y": record.k1_y,
            "Midpoint x": record.midpoint_x,
            "Midpoint y": record.midpoint_y,
            "K2_y": record.k2_y,
            "Midpoint Increment y": record.midpoint_increment_y,
            ...
            "Midpoint Operation": record.midpoint_operation,
            "Update Operation": record.update_operation,
            ...
            row.update(
                {
                    "yp_n": record.yp_n,
                    "K1_yp": record.k1_yp,
                    "Midpoint yp": record.midpoint_yp,
                    "K2_yp": record.k2_yp,
                    "Midpoint Increment yp": record.midpoint_increment_yp,
            ...
            row.update(
                {
                    ...
                    "Midpoint ypp": record.midpoint_ypp,
                    "K2_ypp": record.k2_ypp,
                    "Midpoint Increment ypp": record.midpoint_increment_ypp,
...
def method_formula_dataframe(
    ...
    """Return the method and system formulas."""
    ...
    formulas: list[tuple[str, str]] = [
        ...
        ("System Form", "U' = G(x,U)"),
        ("First Slope", "K1 = G(x_n,U_n)"),
        ("Midpoint State", "U_mid = U_n + (h/2)*K1"),
        ("Second Slope", "K2 = G(x_n+h/2,U_mid)"),
        ("Full-Step Update", "U_(n+1) = U_n + h*K2"),
        ("Expected Global Order", "O(h^2)"),
        ("Expected Local Truncation Error", "O(h^3) per step"),
    ]
...
def create_solution_figure(
    ...
    """Plot y, midpoint points, and the optional exact y solution."""
    ...
    axis.set_title(
        f"Midpoint Method Numerical Solution — "
        f"{order_label(result.ode_order)}"
    )
    axis.set_xlabel("x")
    axis.set_ylabel("y")
    axis.grid(True)
...
def create_state_figure(
    ...
    axis.axhline(0.0, linewidth=1)
    axis.axvline(0.0, linewidth=1)
    axis.set_title("Midpoint Method Derivative-State Components")
    axis.set_xlabel("x")
...
def create_error_figure(
    ...
    axis.set_title("Midpoint Method Global Error in y")
    axis.set_xlabel("x")
...
def create_convergence_figure(
    ...
    axis.set_title(
        "Midpoint Method Grid-Refinement Convergence"
    )
    axis.set_xlabel("|h|")
...
def final_state_dataframe(
    ...
    if result.ode_order >= 2:
        rows.append(
            {
                "Component": "y′",
                "Midpoint Value": result.final_yp,
    ...
    if result.ode_order >= 3:
        rows.append(
            {
                "Component": "y″",
                "Midpoint Value": result.final_ypp,
...
def generate_excel_report(
    ...
    if not result.success:
        raise ValueError(
            "Only successful Midpoint results can be exported."
        )
    ...
    summary_rows = [
        ...
        ("Final Midpoint y", result.final_y),
        ("Final Midpoint y′", result.final_yp),
        ("Final Midpoint y″", result.final_ypp),
        (
            "Exact Solution",
        ...
        ("Expected Global Order", 2),
        ("Expected Local Error Order", 3),
        (
    ...
    with pd.ExcelWriter(
        ...
        if not solution_df.empty:
            ...
            chart = LineChart()
            chart.title = "Midpoint Numerical Solution y"
            ...
            y_column = (
                solution_df.columns.get_loc("Midpoint y") + 1
            )
        ...
        if len(convergence_df) > 1:
            ...
            chart = LineChart()
            chart.title = (
                "Midpoint Grid-Refinement Convergence"
            )
...
def render_final_result(
    ...
    """Render the final updated state and error metrics."""
...
def render_method_summary(
    ...
    """Render the vector Midpoint formulas and problem setup."""
    ...
    with column_1:
        st.latex(
            r"\mathbf{K}_1="
            r"\mathbf{G}(x_n,\mathbf{U}_n)"
        )
        st.latex(
            r"\mathbf{U}_{n+\frac12}="
            r"\mathbf{U}_n+\frac{h}{2}\mathbf{K}_1"
        )
    ...
    with column_2:
        ...
        st.latex(
            r"\mathbf{U}_{n+1}="
            r"\mathbf{U}_n+h\mathbf{K}_2"
        )

    st.dataframe(
        round_numeric_dataframe(
            method_formula_dataframe(result)
        ),
...
def render_iteration_table(
    ...
    """Render every midpoint and update step."""

    st.subheader("Midpoint Iteration Table")
    ...
    st.caption(
        "The half-step midpoint estimate is used only to evaluate K2. "
        "The accepted state is the full-step update Uₙ₊₁ = Uₙ + hK₂. "
        "Local truncation error is calculated by starting one "
        "Midpoint step from the exact state, when an exact solution "
        "is supplied."
    )

    with st.expander(
        "Detailed Midpoint-Update Operations"
    ):
        for record in result.iterations:
            st.markdown(
            ...
            st.code(
                "K1 and midpoint:\n"
                + record.midpoint_operation
                + "\n\nK2 and full-step update:\n"
...
def render_convergence_analysis(
    ...
    """Render second-order convergence analysis."""

    st.subheader("Convergence Analysis")
    st.markdown(
        "The explicit Midpoint RK2 method has "
        "**second-order global convergence** under the usual "
        "smoothness and stability conditions. Halving the step "
        "size should reduce the asymptotic global error by "
...
def render_excel_download(
    ...
    if (
        st.session_state.get(
            "midpoint_excel_signature"
        )
        != report_signature
        or "midpoint_excel_report"
        not in st.session_state
    ):
        try:
            st.session_state.midpoint_excel_report = (
                generate_excel_report(result)
            )
            st.session_state.midpoint_excel_signature = (
                report_signature
            )
        except (
            ValueError,
    ...
    filename = (
        f"midpoint_method_report_{timestamp}.xlsx"
    )
...
def render_page() -> None:
    """Render the complete Midpoint Method Streamlit page."""

    st.set_page_config(
        page_title="Midpoint Method Solver | Numerical Methods",
        page_icon="∫",
        layout="wide",
    )
    load_css()
    ...
    st.html(
        ...
        <section class="solver-hero">
            <div>
                ...
                <h1>Midpoint Method Solver</h1>
                <p>
                    Solve first-, second-, or third-order initial-value problems
                    using the explicit Midpoint RK2 method. Each step evaluates
                    the system slope at a half-step state before the full update.
                    Higher-order equations are converted internally to
                    equivalent systems of first-order ODEs.
                </p>

                <div class="method-actions">
                    <a href="/Midpoint_Method" target="_self"
                       class="btn-outline-ui">Review Lesson →</a>
                    <a href="/Midpoint_Quiz" target="_self"
        ...
        with conditions_column:
            with st.container(border=True):
                ...
                st.markdown(
                    ...
                    - The method forms a half-step state with **K1**, evaluates **K2** there, then updates the complete state with **h·K2**.
            ...
            with st.container(border=True):
                ...
                with second_row[1]:
                    ...
                    convergence_levels = st.number_input(
                        ...
                        step=1,
                        label_visibility="collapsed",
                        key="midpoint_convergence_levels",
                    )
                ...
                solve_clicked = st.button(
                    ...
                    type="primary",
                    use_container_width=True,
                    key="midpoint_solve_button",
                )

                if solve_clicked:
                    result = solve_midpoint_method(
                    ...
                    st.session_state.midpoint_result = result
                    st.session_state[
                        "midpoint_result_signature"
                    ] = current_signature
            ...
            with st.container(border=True):
                ...
                if saved_result is None:
                    ...
                    st.warning(
                        ...
                        "Solve with Midpoint Method to update the result."
                    )
                else:
    ...
    st.html(
        ...
        <footer class="footer-ui">
            ...
            <div>Midpoint Method • Ordinary Differential Equations</div>
        </footer>
        """
    )
